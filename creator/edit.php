...
try {
    ...
    $stmt = db()->prepare(
        "SELECT * FROM dbProj_reviews WHERE review_id = :review_id AND user_id = :user_id LIMIT 1"
    );
    $stmt->execute([
        ':review_id' => $reviewId,
        ':user_id' => $user['id'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $book_id = trim((string) ($_POST['book_id'] ?? ''));
    ...
    } elseif (strlen($review_title) > 200) {
        $errors[] = 'Review title must be 200 characters or less.';
    }

    if (strlen($review_content) < 30) {
        $errors[] = 'Review content must be at least 30 characters.';
    }

    $coverPath = upload_file('cover_image', 'covers', ['image/jpeg', 'image/png', 'image/webp'], $errors) ?? $review['cover_image'];
    $mediaPath = upload_file('media_file', 'media', ['audio/mpeg', 'audio/wav', 'video/mp4', 'video/webm'], $errors) ?? $review['media_file'];
    $downloadPath = upload_file(
        'downloadable_file',
        'downloads',
        ['application/pdf', 'text/plain', 'application/zip'],
        $errors
    ) ?? $review['downloadable_file'];
    ...
    if ($errors === []) {
        try {
            ...
            header('Location: index.php?message=Review updated successfully.');
            exit;
        } catch (PDOException $exception) {
            $errors[] = 'Review could not be updated. Try again later.';
        }
    }
}
...
page_header('Edit Review', '../');
?>

<section class="page-intro">
    <h1>Edit Review</h1>
    <p>Update your review details. You can publish it once you're done.</p>
</section>

<?php if ($errors !== []): ?>
    <section class="notice notice-error" aria-labelledby="edit-errors-title">
        <h2 id="edit-errors-title">Fix these errors</h2>
        <ul>
...
<form class="form-panel form-panel-wide" id="review-form" method="post" enctype="multipart/form-data" novalidate>
    <div class="form-field">
        <label for="book_id">Book</label>
        <select id="book_id" name="book_id" required>
            <option value="">Select a book</option>
            <?php foreach ($books as $book): ?>
                <option value="<?= e((string) $book['book_id']) ?>" <?= (int)$review['book_id'] === (int)$book['book_id'] ? 'selected' : '' ?>>
    ...
    <div class="form-field">
        ...
        <textarea id="review_content" name="review_content" rows="9" minlength="30" required><?= e($review['review_content']) ?></textarea>
    ...
    <div class="form-field">
        ...
        <?php if ($review['cover_image']): ?>
            <p>Current: <a href="../<?= e($review['cover_image']) ?>" target="_blank">View File</a></p>
        <?php endif; ?>
        <input type="file" id="cover_image" name="cover_image" accept="image/jpeg,image/png,image/webp">
        <p class="field-hint">Leave blank to keep current. JPG, PNG, or WebP. Maximum 10MB.</p>
    </div>

    <div class="form-field">
        <label for="media_file">Media file</label>
        <?php if ($review['media_file']): ?>
            <p>Current: <a href="../<?= e($review['media_file']) ?>" target="_blank">View File</a></p>
        <?php endif; ?>
        <input type="file" id="media_file" name="media_file" accept="audio/mpeg,audio/wav,video/mp4,video/webm">
        <p class="field-hint">Leave blank to keep current. MP3, WAV, MP4, or WebM. Maximum 10MB.</p>
    </div>

    <div class="form-field">
        <label for="downloadable_file">Downloadable file</label>
        <?php if ($review['downloadable_file']): ?>
            <p>Current: <a href="../<?= e($review['downloadable_file']) ?>" target="_blank">View File</a></p>
        <?php endif; ?>
        <input type="file" id="downloadable_file" name="downloadable_file" accept="application/pdf,text/plain,application/zip">
        <p class="field-hint">Leave blank to keep current. PDF, TXT, or ZIP. Maximum 10MB.</p>
    </div>
